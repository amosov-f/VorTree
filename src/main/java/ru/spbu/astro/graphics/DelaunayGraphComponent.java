...
public class DelaunayGraphComponent extends Component {
    ...
    protected void paintDelaunayGraph(AbstractDelaunayGraphBuilder.AbstractDelaunayGraph graph, Graphics g, final Color color, int width) {
        Rectangle rect = graph.getRectangle();

        ((Graphics2D) g).setStroke(new BasicStroke(width));

        for (AbstractDelaunayGraphBuilder.Edge edge : graph.getPointEdges()) {
    ...
    protected void paintTriangle(
            final AbstractDelaunayGraphBuilder.Triangle triangle,
            AbstractDelaunayGraphBuilder.AbstractDelaunayGraph graph,
            Graphics g,
            Color color
    ) {
        Rectangle rect = graph.getRectangle();
    ...
    protected void paintTriangle(
            ...
            AbstractDelaunayGraphBuilder.AbstractDelaunayGraph graph,
            Graphics g) {
        paintTriangle(triangle, graph, g, Color.getHSBColor(240f / 360, 1f, 1 - Math.min(triangle.getLevel() * 30f / 255, 1f)));
    }
